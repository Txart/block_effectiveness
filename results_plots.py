...
#%% colormap
cmap_pairs = cm.get_cmap('Paired') # see https://matplotlib.org/3.5.0/tutorials/colors/colormaps.html
...
# Foldernames
modes = ([
        {'foldername': 'no_blocks_dry', 'name':'el Niño, no blocks', 'color':cmap_pairs(6)},
        {'foldername': 'no_blocks_wet', 'name':'wet, no blocks', 'color':cmap_pairs(0)},
        {'foldername': 'yes_blocks_dry', 'name':'el Niño, blocks', 'color':cmap_pairs(7)},
        {'foldername': 'yes_blocks_wet', 'name':'wet, blocks', 'color':cmap_pairs(1)},
        {'foldername': 'yes_blocks', 'name':'weather_station, blocks', 'color':'black'}
        ])

# Get raster dimensions from first raster file
template_rfn = Path(f'hydro_for_plots/params_number_1/{modes[0]["foldername"]}/zeta_after_1_DAYS.tif')
...
def read_data(n_params, modes, n_days, rasters_shape):
    ...
    data = np.zeros(shape=(n_params, len(modes), n_days, rasters_shape[0], rasters_shape[1]))
    for n_param in range(N_PARAMS):
        param_folder = Path(f'hydro_for_plots/params_number_{int(n_param + 1)}')
        for n_mode, mode_foldername in enumerate ([mode['foldername'] for mode in modes]):
            ...
            for day in range(0, N_DAYS):
                fname = foldername.joinpath(f'zeta_after_{int(day+1)}_DAYS.tif')
...
for i_param, param_name in enumerate(params_to_plot):
    for i_mode, mode in enumerate(modes):
        arr = np.zeros(shape=(N_DAYS, len(df_spatialaverage_cols)), dtype=object)
        ...
        df_spatialaverage = pd.concat([df_spatialaverage, new_df], ignore_index=True)
df_spatialaverage['avg_zeta'] = df_spatialaverage['avg_zeta'].astype('float')

# Daily spatial averages
df_daily_spatialaverage_cols = ['day', 'avg_zeta', 'weather', 'param', 'blocks', 'mode']
df_daily_spatialaverage = pd.DataFrame(columns=df_daily_spatialaverage_cols) 
for i_param, param_name in enumerate(params_to_plot):
    for i_mode, mode in enumerate(modes):
        arr = np.zeros(shape=(N_DAYS, len(df_daily_spatialaverage_cols)), dtype=object)
        for i, avg_zeta in np.ndenumerate(spatial_average[i_param, i_mode, :]):
            arr[i,0] = i
            arr[i,1] = avg_zeta
            arr[i,3] = param_name
        ...
        df_daily_spatialaverage = pd.concat([df_daily_spatialaverage, new_df], ignore_index=True)
df_daily_spatialaverage['avg_zeta'] = df_daily_spatialaverage['avg_zeta'].astype('float')
...
for param_to_plot in params_to_plot:
    ...
    for i_mode, mode in enumerate(modes): 
        plt.plot(spatial_average[int(param_to_plot - 1), i_mode], label=mode['name'], color=mode['color'])
    plt.legend()
    plt.show()
...
for i_param, param_name in enumerate(params_to_plot):
    with plt.style.context(("seaborn-paper",)):
        for i_mode, mode in enumerate(modes):
            plt.scatter(i_param + 1, spatiotemporal_average[i_param, i_mode], label=mode['name'], color=mode['color'])
...
df_spatialaverage = df_spatialaverage[df_spatialaverage.weather != 'weather_station']

# Show means 
sns.pointplot(
        data=df_spatialaverage, x='weather', y='avg_zeta', hue='mode',
        palette={mode['name']:mode['color'] for mode in modes[:-1]},
