...
def h_from_theta(theta, dem, s2):
    return np.log(s2* np.exp(s2*dem) * theta + 1)/s2

def theta_from_h(h, dem, s2):
    return 1/s2 * (np.exp(s2*(h - dem)) - np.exp(-s2*dem))
...
def log_prior(params):
    ...
    if -10<s2<10 and 0<t1<1000 and 0<t2<100: 
        return 0.0
    return -np.inf        
...
def log_likelihood(params):
    ...
    for name in relevant_transects:
        ...
        theta = theta_from_h(h_ini, dem, s2)
        ...
        try:
            ...
            solution_zeta = solve_transient_both_diri(theta=theta,
                                                ...
                                                dem=dem, b=b, s1=1.0, s2=s2, t1=t1, t2=t2, SOURCE=SOURCE,
            ...
            print(f"EXECPTION computing hydro with params {params}: {e} Returning infinity")
            return -np.inf 
